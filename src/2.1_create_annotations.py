...
parser.add_argument('--iteration', type=str, help='Iteration identifier', required=True)
parser.add_argument('--start', type=int, help='sample to start at', required=True)
parser.add_argument('--batch_size', type=int, help='number of samples', required=True)
parser.add_argument('--dataset', help="dataset to pull from", required=True)
args = parser.parse_args()

model_path="/lustre/fsn1/projects/rech/fmr/uft12cr/models/llama-3.1-8b-instruct"
llm = LLM(model=model_path)

all_samples = pd.read_csv(f'samples/{args.dataset}_samples.csv')
...
all_samples = all_samples.iloc[args.start:args.start + args.batch_size]
complete_list = []
...
df.to_csv(f'annotations/new_llama_annotations_{args.dataset}_{args.iteration}.csv')
